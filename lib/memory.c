...
void kfree(uint64_t v){
    ASSERT((uint64_t)v % PAGE_SIZE == 0);
    ASSERT((uint64_t)v >= (uint64_t)&end);
    ASSERT((uint64_t)v + PAGE_SIZE <= MEMORY_END);
...
bool setup_uvm(uint64_t map, char* file_name) {
    ...
    if (page != NULL) {
        ...
        if (status == true) {
            if (open(file_name, (uint64_t)page) == -1) {
                free_vm(map);
                return false;
            }
        }
