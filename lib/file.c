...
int open(char *file_path, uint64_t destination_addr) {
    uint32_t index;
    uint32_t file_size;
    ...
    if (index != 0xffffffff) {
        ...
        if (read_file(cluster_index, (void*)destination_addr, file_size) == file_size) {
            ret = 0;
        }
    }
